...
async def upload_pet_story(
    ...
    """Process pet story submission with photo.
    ...
    # Validate file
    if not foto:
        raise HTTPException(status_code=400, detail="Foto do pet é obrigatória")
    
    allowed_content_types = {"image/jpeg", "image/jpg", "image/png", "image/webp"}
    if foto.content_type not in allowed_content_types:
        raise HTTPException(
            status_code=400,
            detail=f"Tipo de arquivo inválido: {foto.content_type}. "
    ...
    try:
        ...
        if len(photo_bytes) == 0:
            raise HTTPException(status_code=400, detail="Arquivo da foto está vazio")
        
        # Validate image size (max 10MB)
        if len(photo_bytes) > 10 * 1024 * 1024:
            raise HTTPException(status_code=400, detail="Foto excede o limite de 10MB")
        
        logger.info(f"Received photo: {foto.filename} ({len(photo_bytes)} bytes)")
        ...
        photo_filename = f"foto_{timestamp}{Path(foto.filename).suffix}"
        photo_path = os.path.join(user_temp_dir, photo_filename)
        
        with open(photo_path, "wb") as f:
            f.write(photo_bytes)
        
        logger.info(f"Photo saved temporarily: {photo_path}")
        ...
        logger.info(f"Background task queued for {nome_pet} ({email})")
